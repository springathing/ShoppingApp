...
namespace mblyakherShoppingApp.Controllers
{
    public class OrdersController : Universal
    {
        ...
        public ActionResult Create([Bind(Include = "Id,Completed,Address,City,State,ZipCode,Country,Phone,Total,OrderDate,CustomerId")] Order order, decimal Total)
        {
            if (ModelState.IsValid)
            {
                var user = db.Users.Find(User.Identity.GetUserId());
                order.CustomerId = user.Id; // since we deleted it in the create we need to give it values as we pass it into OrderItems
                order.OrderDate = System.DateTime.Now;
                order.Total = Total;
                db.Orders.Add(order);               
                db.SaveChanges(); // order gets its Id here

                foreach(var item in user.CartItems.ToList())
                {
                    OrderItem orderitem = new OrderItem();
                    orderitem.ItemId = item.ItemId;
                    orderitem.OrderId = order.Id; // which allows this line to be possible
